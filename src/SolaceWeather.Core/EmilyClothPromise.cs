...
/// <summary>Emily's bounded cloth promise. Only explicit game actions can accept or fulfill it.</summary>
public sealed class EmilyClothPromise
{
    public const string ItemId = "(O)428";
    public int Version { get; set; } = 1;
    public bool SeenCloth { get; set; }
    ...
    public bool IsValid() => Version == 1 && Status is "none" or "offered" or "active" or "overdue" or "completed" or "declined" or "abandoned"
        ...
        && (Status == "none" || SeenCloth && OfferedDay.HasValue && LastActionDay >= OfferedDay)
    ...
    public void Observe(string itemId) { if (itemId == ItemId) SeenCloth = true; }
    public bool CanOffer(int day) => IsValid() && SeenCloth && day >= 0 && day >= LastActionDay
