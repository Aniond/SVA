...
public static class RomanceRules
{
    ...
    public static bool StartDating(RomanceSaveState s, string name, int day) { if (!Ready(s, day) || !s.GetJourney(name, day).CanDate) return false; var c = Character(s, name, day); c.IsDating = true; c.DatingSinceDay = day; c.ConflictFreeSinceDay = day; Remember(s, name, day, "dating"); return true; }
    ...
    public static bool LearnIncident(RomanceSaveState s, string incidentId, string knowerNpc, int day, string source, int hops = 0, string? originalEyewitness = null)
    {
        ...
        if (c.WarningDeliveredDay.HasValue && i.Day >= c.WarningDeliveredDay.Value && i.Sequence > c.WarningIncidentSequence && c.SeparationUntilDay == null) c.SeparationUntilDay = day + 14;
