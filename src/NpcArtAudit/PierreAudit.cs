...
// Verifies native appearances, gestures, sleep and Community Center fight commands.
// Runs isolated native appearance, frame, animation and route checks; not full scene playback.
internal static class PierreAudit
{
    public static void Run(IModHelper helper, IMonitor monitor)
    {
        ...
        try
        {
            ...
            NPC Create() => new(new AnimatedSprite("Characters/Pierre",0,16,32),Vector2.Zero,2,"Pierre"){currentLocation=location};
            var npc=Create();var walkingSteps=0;var specialSteps=0;var routes=0;var poseCount=0;var portraitCount=0;var placeholderCount=0;var eventShowFrameCommands=0;var guardSteps=0;var eventAnimateCommands=0;
            foreach(var variant in new[]{"Base","Winter","Beach"})
            {
                ...
                var suffix=variant=="Base"?"":"_"+variant;var spriteAsset="Characters/Pierre"+suffix;var portraitAsset="Portraits/Pierre"+suffix;
                ...
                var texture=npc.Sprite.Texture;var portrait=npc.Portrait;var frameCount=variant=="Beach"?16:25;
                if(texture.Width!=64||texture.Height!=(variant=="Beach"?128:224)||portrait.Width!=128||portrait.Height!=(variant=="Beach"?128:192))throw new Exception("Native atlas size differs");
                ...
                if(variant!="Beach")for(var i=25;i<28;i++){for(var y=0;y<32;y++)for(var x=0;x<16;x++)if(p[(i/4*32+y)*64+i%4*16+x]!=new Color(123,66,26,255))throw new Exception("Native solid placeholder differs");placeholderCount++;}
                for(var i=0;i<(variant=="Beach"?4:6);i++){if(new Dialogue(npc,null,"Expression.$"+i).getPortraitIndex()!=i)throw new Exception("Portrait index differs");var count=0;for(var y=0;y<64;y++)for(var x=0;x<64;x++)if(pp[(i/2*64+y)*128+i%2*64+x].A>0)count++;if(count==0)throw new Exception("Empty portrait");portraitCount++;}
                ...
                if(variant!="Beach")foreach(var key in new[]{"pierre_sleep"})
                {
                ...
                if(variant!="Beach")
                {
                    ...
                    foreach(var map in new[]{"ArchaeologyHouse","CommunityCenter","Farm","Saloon","SeedShop","Town"})
                    ...
                    {
                        var args=command.Split(' ',StringSplitOptions.RemoveEmptyEntries);if(args.Length<3||args[0]!="showFrame"||args[1]!="Pierre")continue;var frame=int.Parse(args[2]);if(frame<0||frame>=25)throw new Exception("Native event references missing pose");npc.Sprite.StopAnimation();npc.Sprite.CurrentFrame=frame==0?1:0;
                    ...
                    foreach(var expected in new[]{16,17,18,19,22,23})if(!seen.Contains(expected))throw new Exception("Native event pose missing: "+expected);
                    var punch=helper.GameContent.Load<Dictionary<string,string>>("Data/Events/CommunityCenter")["Punch"];
                    foreach(var command in punch.Split('/'))
                    {
                        var args=command.Split(' ',StringSplitOptions.RemoveEmptyEntries);if(args.Length<6||args[0]!="animate"||args[1]!="Pierre")continue;
                        ...
                        if(!npc.Sprite.currentAnimation.Select(f=>f.frame).SequenceEqual(frames)||!npc.Sprite.currentAnimation.All(f=>f.milliseconds==duration))throw new Exception("Native guard command differs");
                        npc.Sprite.CurrentFrame=frames[0];npc.Sprite.currentAnimationIndex=0;npc.Sprite.timer=0;
                        for(var step=1;step<=frames.Length*2;step++){npc.Sprite.animateOnce(new GameTime(TimeSpan.FromMilliseconds(step*duration),TimeSpan.FromMilliseconds(duration)));if(npc.Sprite.CurrentFrame!=frames[step%frames.Length])throw new Exception("Guard timed transition differs");guardSteps++;}
                ...
                try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<texture.Height/32*4;i++)batch.Draw(texture,new Rectangle(i%4*64+8,i/4*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(portrait,new Rectangle(300,16,384,portrait.Height*3),Color.White);batch.End();device.SetRenderTargets(previous);using var file=File.Create(Path.Combine(helper.DirectoryPath,"pierre-"+variant.ToLowerInvariant()+"-runtime-preview.png"));target.SaveAsPng(file,target.Width,target.Height);}finally{device.SetRenderTargets(previous);}
            }
            npc.wearNormalClothes();npc.ChooseAppearance();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Pierre"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Pierre"));
            helper.Data.WriteJsonFile("pierre-checks.json",new{Passed=true,OccupiedPoses=poseCount,PreservedPlaceholders=placeholderCount,PortraitSlots=portraitCount,NativeWalkingSteps=walkingSteps,NativeSpecialSteps=specialSteps,NativeRouteTransitions=routes,NativeEventShowFrameCommands=eventShowFrameCommands,NativeGuardTimedSteps=guardSteps,NativeEventAnimateCommands=eventAnimateCommands,NativeWinterBeachTransitions=true,EverydayRestored=true,FarmLoaded=false,FullEventPlayback=false});monitor.Log("Pierre art audit passed; full saved-game scenes remain unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("pierre-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Pierre audit failed: "+ex,LogLevel.Error);}
