...
internal static class SamWinterAudit
{
    public static void Run(IModHelper helper, IMonitor monitor)
    {
        ...
        try
        {
            ...
            if ((npc.Sprite.overrideTextureName ?? npc.Sprite.textureName.Value).Replace('\\', '/') != "Characters/Sam_Winter") throw new Exception("Native winter appearance missing");
            ...
            Equal(npc.Sprite.Texture, helper.GameContent.Load<Texture2D>("Characters/Sam_Winter"));
            Equal(npc.Portrait, helper.GameContent.Load<Texture2D>("Portraits/Sam_Winter"));
            ...
            for (var frame = 0; frame < 55; frame++)
            {
                ...
                if (visible == 0) throw new Exception("Empty Sam winter pose: " + frame);
            }
            ...
            helper.Data.WriteJsonFile("sam-winter-native-events.json", new { Keys = eventKeys, Frames = eventFrames, ActualShowFrameCommands = showCommands, FullEventPlayback = false });
            ...
            var walker = new AnimatedSprite("Characters/Sam_Winter", 0, 16, 32); var walkingSteps = 0;
            ...
            try
            {
                ...
                using var file = File.Create(Path.Combine(helper.DirectoryPath, "sam-winter-runtime-preview.png")); target.SaveAsPng(file, target.Width, target.Height);
            }
            finally { device.SetRenderTargets(previous); }
            Game1.season = Season.Spring; npc.ChooseAppearance(); Equal(npc.Sprite.Texture, helper.GameContent.Load<Texture2D>("Characters/Sam")); Equal(npc.Portrait, helper.GameContent.Load<Texture2D>("Portraits/Sam"));
            helper.Data.WriteJsonFile("sam-winter-checks.json", new { Passed = true, OccupiedPoses = 55, PreservedPlaceholders = 1, PortraitSlots = 12, NativeWinterSelected = true, SpringRestored = true, NativeSpecialSteps = animationSteps, NativeRouteIntroToLoopTransitions = routeTransitions, NativeWalkingSteps = walkingSteps, NativeEventFrameReferences = eventFrames.Count, NativeShowFrameCommands = showCommands, NativeSkateCallbacks = 5, FarmLoaded = false, FullEventPlayback = false });
            monitor.Log("Sam winter audit passed: 55 poses, native placeholder, 12 portraits, native route transitions and event frame commands; full scenes remain unverified.", LogLevel.Info);
        }
        catch (Exception ex) { helper.Data.WriteJsonFile("sam-winter-checks.json", new { Passed = false, Error = ex.ToString() }); monitor.Log("Sam winter audit failed: " + ex, LogLevel.Error); }
