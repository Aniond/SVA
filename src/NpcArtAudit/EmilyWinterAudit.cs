...
internal static class EmilyWinterAudit
{
    public static void Run(IModHelper helper,IMonitor monitor)
    {
        ...
        try
        {
            ...
            if((npc.Sprite.overrideTextureName??npc.Sprite.textureName.Value).Replace('\\','/')!="Characters/Emily_Winter")throw new Exception("Native winter appearance not selected");
            ...
            Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Emily_Winter"));
            Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Emily_Winter"));
            for(var i=0;i<8;i++){var dialogue=new Dialogue(npc,null,"Winter portrait check.$"+i);if(dialogue.getPortraitIndex()!=i)throw new Exception("Portrait index mismatch "+i);}
            var texture=npc.Sprite.Texture;var pixels=new Color[texture.Width*texture.Height];texture.GetData(pixels);
            for(var frame=0;frame<56;frame++){npc.Sprite.CurrentFrame=frame;var rect=npc.Sprite.SourceRect;if(rect.X!=frame%4*16||rect.Y!=frame/4*32)throw new Exception("Frame layout mismatch");var visible=0;for(var y=rect.Y;y<rect.Bottom;y++)for(var x=rect.X;x<rect.Right;x++)if(pixels[y*64+x].A>0)visible++;if((frame<56)!=(visible>0))throw new Exception("Occupied/blank frame mismatch");}
            var animationData=helper.GameContent.Load<Dictionary<string,string>>("Data/animationDescriptions");
            var animationSteps=0;
            foreach(var key in new[]{"emily_exercise","emily_sleep"})
            {
                var frames=animationData[key].Split('/')[1].Split(' ',StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();
                if(frames.Any(frame=>frame<0||frame>=56))throw new Exception("Native animation references invalid frame");
            ...
            try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<56;i++)batch.Draw(texture,new Rectangle(i%8*64+8,i/8*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(npc.Portrait,new Rectangle(600,16,384,768),Color.White);batch.End();device.SetRenderTargets(previous);using var f=File.Create(Path.Combine(helper.DirectoryPath,"emily-winter-runtime-preview.png"));target.SaveAsPng(f,1024,1024);}finally{device.SetRenderTargets(previous);}
            Game1.season=Season.Spring;npc.ChooseAppearance();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Emily"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Emily"));
            helper.Data.WriteJsonFile("emily-winter-checks.json",new{Passed=true,OccupiedFrames=56,BlankFrames=0,PortraitSlots=8,NativeWinterSelected=true,SpringRestored=true,NativeExerciseAndSleepSteps=animationSteps,FarmLoaded=false,FullEventPlayback=false});
            monitor.Log("Emily winter audit passed:56 poses,8 portrait slots, native winter selection and spring restoration; event playback remains unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("emily-winter-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Emily winter audit failed: "+ex,LogLevel.Error);}
