...
internal static class LeahBaseAudit
{
    public static void Run(IModHelper helper,IMonitor monitor)
    {
        ...
        try
        {
            ...
            if((npc.Sprite.overrideTextureName??npc.Sprite.textureName.Value).Replace('\\','/')!="Characters/Leah")throw new Exception("Native everyday appearance not selected");
            ...
            Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Leah"));
            Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Leah"));
            ...
            var portraitPixels=new Color[npc.Portrait.Width*npc.Portrait.Height];npc.Portrait.GetData(portraitPixels);
            for(var index=0;index<10;index++){var visible=false;for(var y=0;y<64;y++)for(var x=0;x<64;x++)if(portraitPixels[(index/2*64+y)*npc.Portrait.Width+index%2*64+x].A>0)visible=true;if(!visible)throw new Exception("Empty Leah portrait expression "+index);}
            ...
            var walkingSteps = 0;
            foreach (var asset in new[] { "Characters/Leah", "Characters/Leah_Winter", "Characters/Leah_Beach" })
            {
            ...
            try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<52;i++)batch.Draw(texture,new Rectangle(i%8*64+8,i/8*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(npc.Portrait,new Rectangle(600,16,384,960),Color.White);batch.End();device.SetRenderTargets(previous);using var f=File.Create(Path.Combine(helper.DirectoryPath,"leah-base-runtime-preview.png"));target.SaveAsPng(f,target.Width,target.Height);}finally{device.SetRenderTargets(previous);}
            Game1.season=Season.Winter;npc.ChooseAppearance();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Leah_Winter"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Leah_Winter"));
            helper.Data.WriteJsonFile("leah-base-checks.json",new{Passed=true,OccupiedFrames=50,BlankFrames=2,PortraitSlots=10,NativeBaseSelected=true,WinterSelectedAfter=true,NativeSculptDrawSleepSteps=animationSteps,NativeWalkingSteps=walkingSteps,WalkingOutfits=3,FarmLoaded=false,FullEventPlayback=false});
            monitor.Log("Leah everyday audit passed:50 poses,10 portrait slots, native everyday selection and winter transition; event playback remains unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("leah-base-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Leah everyday audit failed: "+ex,LogLevel.Error);}
