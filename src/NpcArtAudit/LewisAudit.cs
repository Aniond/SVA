...
// Integrated after coordinator artwork and native-source review.
// Runs isolated native appearance, frame, animation and route checks; not full scene playback.
internal static class LewisAudit
{
    public static void Run(IModHelper helper, IMonitor monitor)
    {
        ...
        try
        {
            ...
            NPC Create() => new(new AnimatedSprite("Characters/Lewis",0,16,32),Vector2.Zero,2,"Lewis"){currentLocation=location};
            var npc=Create();var walkingSteps=0;var specialSteps=0;var routes=0;var poseCount=0;var portraitCount=0;var placeholderCount=0;var eventShowFrameCommands=0;var eventAnimateCommands=0;
            foreach(var variant in new[]{"Base","Winter","Beach"})
            {
                ...
                var suffix=variant=="Base"?"":"_"+variant;var spriteAsset="Characters/Lewis"+suffix;var portraitAsset="Portraits/Lewis"+(variant=="Winter"?"_Winter":"");
                ...
                var texture=npc.Sprite.Texture;var portrait=npc.Portrait;var frameCount=variant=="Beach"?16:29;
                if(texture.Width!=64||texture.Height!=(variant=="Beach"?128:256)||portrait.Width!=128||portrait.Height!=192)throw new Exception("Native atlas size differs");
                ...
                if(variant!="Beach")for(var i=29;i<32;i++){for(var y=0;y<32;y++)for(var x=0;x<16;x++){var transparent=i==30&&((y==6&&x>=7&&x<=14)||(y>=7&&y<=9&&x>=7)||(y>=10&&y<=11&&x>=7&&x<=14))||i==31&&x==0&&(y==8||y==9);if(p[(i/4*32+y)*64+i%4*16+x]!=(transparent?Color.Transparent:Color.White))throw new Exception("Native white/transparent placeholder differs");}placeholderCount++;}
                ...
                if(variant!="Beach")foreach(var key in new[]{"lewis_garden","lewis_drink","lewis_sleep"})
                {
                ...
                if(variant!="Beach")
                {
                    ...
                    foreach(var map in new[]{"ArchaeologyHouse","BusStop","ManorHouse","Saloon","Town"})
                    ...
                    {
                        var args=command.Split(' ',StringSplitOptions.RemoveEmptyEntries);if(args.Length<3||args[0]!="showFrame"||args[1]!="Lewis")continue;var frame=int.Parse(args[2]);if(frame<0||frame>=29)throw new Exception("Native event references missing pose");npc.Sprite.StopAnimation();npc.Sprite.CurrentFrame=frame==0?1:0;
                    ...
                    foreach(var expected in new[]{20,21,24,26,28})if(!seen.Contains(expected))throw new Exception("Native event pose missing: "+expected);
                    foreach(var script in helper.GameContent.Load<Dictionary<string,string>>("Data/Events/Town").Values)foreach(var command in script.Split('/'))
                    {
                        var args=command.Split(' ',StringSplitOptions.RemoveEmptyEntries);if(args.Length<6||args[0]!="animate"||args[1]!="Lewis")continue;
                        ...
                        if(!npc.Sprite.currentAnimation.Select(f=>f.frame).SequenceEqual(frames)||!npc.Sprite.currentAnimation.All(f=>f.milliseconds==duration))throw new Exception("Native Animate command differs");eventAnimateCommands++;
                    }
                }
                var device=Game1.graphics.GraphicsDevice;var previous=device.GetRenderTargets();using var target=new RenderTarget2D(device,768,1070);using var batch=new SpriteBatch(device);
                try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<texture.Height/32*4;i++)batch.Draw(texture,new Rectangle(i%4*64+8,i/4*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(portrait,new Rectangle(300,16,384,portrait.Height*3),Color.White);batch.End();device.SetRenderTargets(previous);using var file=File.Create(Path.Combine(helper.DirectoryPath,"lewis-"+variant.ToLowerInvariant()+"-runtime-preview.png"));target.SaveAsPng(file,target.Width,target.Height);}finally{device.SetRenderTargets(previous);}
            }
            Game1.season=Season.Spring;npc.wearNormalClothes();npc.ChooseAppearance();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Lewis"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Lewis"));
            helper.Data.WriteJsonFile("lewis-checks.json",new{Passed=true,OccupiedPoses=poseCount,PreservedPlaceholders=placeholderCount,PortraitSlots=portraitCount,NativeWalkingSteps=walkingSteps,NativeSpecialSteps=specialSteps,NativeRouteTransitions=routes,NativeEventShowFrameCommands=eventShowFrameCommands,NativeEventAnimateCommands=eventAnimateCommands,NativeWinterBeachTransitions=true,EverydayRestored=true,FarmLoaded=false,FullEventPlayback=false});monitor.Log("Lewis art audit passed; full saved-game scenes remain unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("lewis-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Lewis audit failed: "+ex,LogLevel.Error);}
