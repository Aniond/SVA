...
internal static class AlexBaseAudit
{
    public static void Run(IModHelper helper,IMonitor monitor)
    {
        ...
        try
        {
            ...
            if((npc.Sprite.overrideTextureName??npc.Sprite.textureName.Value).Replace('\\','/')!="Characters/Alex")throw new Exception("Native everyday appearance not selected");
            ...
            Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Alex"));
            Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Alex"));
            ...
            helper.Data.WriteJsonFile("alex-base-native-event.json",new{Key=nativeEvent.Key,Frames=eventFrames,FullEventPlayback=false});
            ...
            var walkingSteps = 0;
            foreach (var asset in new[] { "Characters/Alex", "Characters/Alex_Winter", "Characters/Alex_Beach" })
            {
            ...
            try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<52;i++)batch.Draw(texture,new Rectangle(i%8*64+8,i/8*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(npc.Portrait,new Rectangle(600,16,384,1152),Color.White);batch.End();device.SetRenderTargets(previous);using var f=File.Create(Path.Combine(helper.DirectoryPath,"alex-base-runtime-preview.png"));target.SaveAsPng(f,target.Width,target.Height);}finally{device.SetRenderTargets(previous);}
            Game1.season=Season.Winter;npc.ChooseAppearance();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Alex_Winter"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Alex_Winter"));
            helper.Data.WriteJsonFile("alex-base-checks.json",new{Passed=true,OccupiedArtworkFrames=51,BlankFrames=1,PortraitSlots=12,NativeEverydaySelected=true,WinterSelected=true,NativeFootballWeightsSitSleepSteps=animationSteps,NativeWalkingSteps=walkingSteps,NativeMusicBoxEventFrames=eventFrames.Length,FarmLoaded=false,FullEventPlayback=false});
            monitor.Log("Alex everyday audit passed:51 poses,1 blank,12 portrait slots, native everyday and winter selection; event playback remains unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("alex-base-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Alex everyday audit failed: "+ex,LogLevel.Error);}
