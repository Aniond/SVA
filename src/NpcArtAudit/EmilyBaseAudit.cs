...
internal static class EmilyBaseAudit
{
    public static void Run(IModHelper helper,IMonitor monitor)
    {
        ...
        try
        {
            ...
            if((npc.Sprite.overrideTextureName??npc.Sprite.textureName.Value).Replace('\\','/')!="Characters/Emily")throw new Exception("Native base appearance not selected");
            ...
            Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Emily"));
            Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Emily"));
            for(var i=0;i<8;i++){var dialogue=new Dialogue(npc,null,"Base portrait check.$"+i);if(dialogue.getPortraitIndex()!=i)throw new Exception("Portrait index mismatch "+i);}
            ...
            try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<56;i++)batch.Draw(texture,new Rectangle(i%8*64+8,i/8*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(npc.Portrait,new Rectangle(600,16,384,768),Color.White);batch.End();device.SetRenderTargets(previous);using var f=File.Create(Path.Combine(helper.DirectoryPath,"emily-base-runtime-preview.png"));target.SaveAsPng(f,1024,1024);}finally{device.SetRenderTargets(previous);}
            Game1.season=Season.Winter;npc.ChooseAppearance();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Emily_Winter"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Emily_Winter"));
            helper.Data.WriteJsonFile("emily-base-checks.json",new{Passed=true,OccupiedFrames=56,BlankFrames=0,PortraitSlots=8,NativeBaseSelected=true,WinterSelectedAfter=true,NativeExerciseAndSleepSteps=animationSteps,FarmLoaded=false,FullEventPlayback=false});
            monitor.Log("Emily base audit passed:56 poses,8 portrait slots, native base selection and winter transition; event playback remains unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("emily-base-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Emily base audit failed: "+ex,LogLevel.Error);}
