...
internal static class MaruBeachAudit
{
    public static void Run(IModHelper helper,IMonitor monitor)
    {
        ...
        try
        {
            ...
            if((npc.Sprite.overrideTextureName??npc.Sprite.textureName.Value).Replace('\\','/')!="Characters/Maru_Beach")throw new Exception("Native beach appearance not selected");
            ...
            Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Maru_Beach"));
            Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Maru_Beach"));
            ...
            foreach(var key in new[]{"maru_beach_towel"})
            ...
            foreach(var key in new[]{"maru_beach_towel"})
            {
                var towelNpc=new NPC(new AnimatedSprite("Characters/Maru",0,16,32),Vector2.Zero,2,"Maru"){currentLocation=location};
                ...
                var expectedOffset=new Vector2(0,16);
            ...
            var walkingSteps = 0;
            foreach (var asset in new[] { "Characters/Maru_Beach" })
            {
            ...
            try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<20;i++)batch.Draw(texture,new Rectangle(i%8*64+8,i/8*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(npc.Portrait,new Rectangle(600,16,384,960),Color.White);batch.End();device.SetRenderTargets(previous);using var f=File.Create(Path.Combine(helper.DirectoryPath,"maru-beach-runtime-preview.png"));target.SaveAsPng(f,1024,1024);}finally{device.SetRenderTargets(previous);}
            npc.wearNormalClothes();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Maru"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Maru"));
            helper.Data.WriteJsonFile("maru-beach-checks.json",new{Passed=true,OccupiedFrames=20,BlankFrames=0,PortraitSlots=10,NativeBeachSelected=true,NormalOutfitRestored=true,NativeTowelSteps=animationSteps,NativeWalkingSteps=walkingSteps,NativeTowelBehaviors=nativeTowelBehaviors,RecliningSceneVerified=false,FarmLoaded=false,FullEventPlayback=false});
            monitor.Log("Maru beach audit passed:20 poses,10 portrait slots, native beach selection and normal outfit restoration; event playback remains unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("maru-beach-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Maru beach audit failed: "+ex,LogLevel.Error);}
