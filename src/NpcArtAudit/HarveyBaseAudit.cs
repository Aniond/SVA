...
internal static class HarveyBaseAudit
{
    public static void Run(IModHelper helper, IMonitor monitor)
    {
        ...
        try
        {
            ...
            if ((npc.Sprite.overrideTextureName ?? npc.Sprite.textureName.Value).Replace('\\', '/') != "Characters/Harvey") throw new Exception("Native everyday appearance missing");
            ...
            Equal(npc.Sprite.Texture, helper.GameContent.Load<Texture2D>("Characters/Harvey"));
            Equal(npc.Portrait, helper.GameContent.Load<Texture2D>("Portraits/Harvey"));
            ...
            for (var frame = 0; frame < 55; frame++)
            {
                ...
                if (visible == 0) throw new Exception("Empty Harvey everyday pose: " + frame);
            }
            for (var y = 0; y < 32; y++) for (var x = 0; x < 16; x++)
                if (pixels[(416 + y) * 64 + 48 + x] != Color.Black) throw new Exception("Native black placeholder changed");
            var winterPixels = new Color[64 * 448]; helper.GameContent.Load<Texture2D>("Characters/Harvey_Winter").GetData(winterPixels);
            for (var frame = 44; frame <= 50; frame++) for (var y = 0; y < 32; y++) for (var x = 0; x < 16; x++)
            {
                var p = (frame / 4 * 32 + y) * 64 + frame % 4 * 16 + x;
                if (pixels[p] != winterPixels[p]) throw new Exception("Retained modern winter pose differs: " + frame);
            }
            ...
            helper.Data.WriteJsonFile("harvey-base-native-events.json", new { Keys = eventKeys, Frames = eventFrames, ActualShowFrameCommands = showCommands, FullEventPlayback = false });
            ...
            var walkingSteps = 0;
            foreach (var asset in new[] { "Characters/Harvey", "Characters/Harvey_Winter", "Characters/Harvey_Beach" })
            {
            ...
            try
            {
                ...
                using var file = File.Create(Path.Combine(helper.DirectoryPath, "harvey-base-runtime-preview.png")); target.SaveAsPng(file, target.Width, target.Height);
            }
            finally { device.SetRenderTargets(previous); }
            Game1.season = Season.Winter; npc.ChooseAppearance(); Equal(npc.Sprite.Texture, helper.GameContent.Load<Texture2D>("Characters/Harvey_Winter")); Equal(npc.Portrait, helper.GameContent.Load<Texture2D>("Portraits/Harvey_Winter"));
            Game1.season = Season.Summer; npc.wearIslandAttire(); Equal(npc.Sprite.Texture, helper.GameContent.Load<Texture2D>("Characters/Harvey_Beach")); Equal(npc.Portrait, helper.GameContent.Load<Texture2D>("Portraits/Harvey_Beach"));
            ...
            helper.Data.WriteJsonFile("harvey-base-checks.json", new { Passed = true, OccupiedPoses = 55, PreservedPlaceholders = 1, PortraitSlots = 12, NativeEverydaySelected = true, WinterAndBeachTransitions = true, NormalRestored = true, RetainedWinterFrames = 7, NativeSpecialSteps = animationSteps, NativeRouteIntroToLoopTransitions = routeTransitions, NativeWalkingSteps = walkingSteps, NativeEventFrameReferences = eventFrames.Count, NativeShowFrameCommands = showCommands, FarmLoaded = false, FullEventPlayback = false });
            monitor.Log("Harvey everyday audit passed: 55 poses, native placeholder, 12 portraits, native route transitions and event frame commands; full scenes remain unverified.", LogLevel.Info);
        }
        catch (Exception ex) { helper.Data.WriteJsonFile("harvey-base-checks.json", new { Passed = false, Error = ex.ToString() }); monitor.Log("Harvey everyday audit failed: " + ex, LogLevel.Error); }
