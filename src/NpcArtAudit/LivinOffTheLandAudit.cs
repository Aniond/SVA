...
internal static class LivinOffTheLandAudit
{
    public static void Run(IModHelper helper,IMonitor monitor)
    {
        ...
        var recipes=Game1.player.cookingRecipes.Keys.ToDictionary(k=>k,k=>Game1.player.cookingRecipes[k]);var oldDay=Game1.dayOfMonth;var oldDaysPlayed=Game1.stats.DaysPlayed;
        ...
        try
        {
            Game1.currentLocation=new GameLocation("Maps/FarmHouse","FarmHouse");Game1.dayOfMonth=7;Game1.stats.DaysPlayed=DataLoader.Tv_TipChannel(Game1.temporaryContent).Keys.Select(uint.Parse).First(k=>k>0 && k<224);
            ...
            object CheckPanel(DialogueBox box,int expression){var args=new object?[]{box,null};if(!(bool)table.GetType().GetMethod("TryGetValue")!.Invoke(table,args)!)throw new Exception("Missing Livin' Off the Land portrait");var p=args[1]!;if((string)p.GetType().GetProperty("Name")!.GetValue(p)! != "Livin' Off the Land"||(int)p.GetType().GetProperty("Emotion")!.GetValue(p)! != expression)throw new Exception("Queen portrait identity or expression differs");return p;}
            string Normalize(string value)=>string.Concat(value.Where(c=>!char.IsWhiteSpace(c)));
            menuField.SetValue(null,null);Game1.dialogueUp=false;var tv=(TV)ItemRegistry.Create("(F)1468");tv.selectChannel(Game1.player,"Livin'");
            var opening=Game1.activeClickableMenu as DialogueBox??throw new Exception("Native opening absent");CheckPanel(opening,0);
            if(Normalize(opening.getCurrentString())!=Normalize(Game1.content.LoadString("Strings\\StringsFromCSFiles:TV.cs.13124"))||Game1.afterDialogues?.Target!=tv)throw new Exception("Native opening or callback changed");
            var screen=helper.Reflection.GetField<TemporaryAnimatedSprite>(tv,"screen").GetValue();
            if(screen.sourceRect!=new Rectangle(517,361,42,28)||screen.animationLength!=2||screen.interval!=150)throw new Exception("Native chef TV geometry differs");
            var expectedText=new[]{(string)typeof(TV).GetMethod("getTodaysTip",BindingFlags.Instance|BindingFlags.NonPublic,null,Type.EmptyTypes,null)!.Invoke(tv,null)!};
            var expectedRecipes=Game1.player.cookingRecipes.Keys.ToDictionary(k=>k,k=>Game1.player.cookingRecipes[k]);RestoreRecipes();
            Game1.afterDialogues!();var recipe=Game1.activeClickableMenu as DialogueBox??throw new Exception("Native recipe absent");var recipePanel=CheckPanel(recipe,1);
            ...
            var portrait=(Texture2D)recipePanel.GetType().GetProperty("Texture")!.GetValue(recipePanel)!;var expectedPortrait=helper.GameContent.Load<Texture2D>("Portraits/LivinOffTheLand");
            ...
            helper.Data.WriteJsonFile("livin-off-the-land-checks.json",new{Passed=true,NativeOpening=true,NativeTipMessages=true,ActualTipDate=Game1.stats.DaysPlayed,RecipeStateUnchanged=true,PortraitPixelsMatch=true,NativeTvTurnOff=true,FarmLoaded=false,FullFurnitureScene=false});
        }
        catch(Exception ex){helper.Data.WriteJsonFile("livin-off-the-land-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Livin' Off the Land audit failed: "+ex,LogLevel.Error);}
        finally{RestoreRecipes();team.lastDayQueenOfSauceRerunUpdated.Value=oldRerunDay;team.queenOfSauceRerunWeek.Value=oldRerunWeek;Game1.dayOfMonth=oldDay;Game1.stats.DaysPlayed=oldDaysPlayed;foreach(var key in Game1.currentLightSources.Keys.Where(k=>!lights.Contains(k)).ToArray())Game1.currentLightSources.Remove(key);menuField.SetValue(null,oldMenu);Game1.currentLocation=oldLocation;Game1.afterDialogues=oldAfter;Game1.dialogueUp=oldDialogue;Game1.player.CanMove=oldMove;}
    }
}
