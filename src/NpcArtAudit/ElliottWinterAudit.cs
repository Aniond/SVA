...
internal static class ElliottWinterAudit
{
    public static void Run(IModHelper helper,IMonitor monitor)
    {
        ...
        try
        {
            ...
            if((npc.Sprite.overrideTextureName??npc.Sprite.textureName.Value).Replace('\\','/')!="Characters/Elliott_Winter")throw new Exception("Native winter appearance not selected");
            ...
            Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Elliott_Winter"));
            Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Elliott_Winter"));
            ...
            helper.Data.WriteJsonFile("elliott-winter-native-events.json",new{Keys=nativeEvents.Select(e=>e.Key),Frames=eventFrames,FullEventPlayback=false});
            ...
            var walkingSteps = 0;
            foreach (var asset in new[] { "Characters/Elliott_Winter" })
            {
            ...
            try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<52;i++)batch.Draw(texture,new Rectangle(i%8*64+8,i/8*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(npc.Portrait,new Rectangle(600,16,384,960),Color.White);for(var fishingPose=0;fishingPose<2;fishingPose++)batch.Draw(texture,new Rectangle(16+fishingPose*180,976,128,128),new Rectangle(fishingPose*32,320,32,32),Color.White);batch.End();device.SetRenderTargets(previous);using var f=File.Create(Path.Combine(helper.DirectoryPath,"elliott-winter-runtime-preview.png"));target.SaveAsPng(f,target.Width,target.Height);}finally{device.SetRenderTargets(previous);}
            Game1.season=Season.Spring;npc.ChooseAppearance();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Elliott"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Elliott"));
            helper.Data.WriteJsonFile("elliott-winter-checks.json",new{Passed=true,OccupiedArtworkCells=51,CompletePoses=49,BlankFrames=1,PortraitSlots=10,NativeWinterSelected=true,SpringRestored=true,NativeReadSitDrinkSleepSteps=animationSteps,NativeWalkingSteps=walkingSteps,NativeEventFrameReferences=eventFrames.Count,NativeFishingLayouts=fishingLayouts,NativeFishingExtendReset=true,FarmLoaded=false,FullEventPlayback=false});
            monitor.Log("Elliott winter audit passed:51 occupied cells,1 blank,10 portrait slots, native winter selection and spring restoration; event playback remains unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("elliott-winter-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Elliott winter audit failed: "+ex,LogLevel.Error);}
