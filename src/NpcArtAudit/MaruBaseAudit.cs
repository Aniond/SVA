...
internal static class MaruBaseAudit
{
    public static void Run(IModHelper helper,IMonitor monitor)
    {
        ...
        try
        {
            ...
            if((npc.Sprite.overrideTextureName??npc.Sprite.textureName.Value).Replace('\\','/')!="Characters/Maru")throw new Exception("Native everyday appearance not selected");
            ...
            Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Maru"));
            Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Maru"));
            ...
            var walkingSteps = 0;
            foreach (var asset in new[] { "Characters/Maru", "Characters/Maru_Winter", "Characters/Maru_Beach", "Characters/Maru_Hospital" })
            {
            ...
            try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<48;i++)batch.Draw(texture,new Rectangle(i%8*64+8,i/8*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(npc.Portrait,new Rectangle(600,16,384,960),Color.White);batch.End();device.SetRenderTargets(previous);using var f=File.Create(Path.Combine(helper.DirectoryPath,"maru-base-runtime-preview.png"));target.SaveAsPng(f,target.Width,target.Height);}finally{device.SetRenderTargets(previous);}
            Game1.season=Season.Winter;npc.ChooseAppearance();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Maru_Winter"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Maru_Winter"));
            helper.Data.WriteJsonFile("maru-base-checks.json",new{Passed=true,OccupiedFrames=45,BlankFrames=3,PortraitSlots=10,NativeBaseSelected=true,WinterSelectedAfter=true,NativeTinkerSitSleepSteps=animationSteps,NativeWalkingSteps=walkingSteps,WalkingOutfits=4,FarmLoaded=false,FullEventPlayback=false});
            monitor.Log("Maru everyday audit passed:45 poses,10 portrait slots, native everyday selection and winter transition; event playback remains unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("maru-base-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Maru everyday audit failed: "+ex,LogLevel.Error);}
