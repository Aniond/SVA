...
internal static class JodiAudit
{
    public static void Run(IModHelper helper, IMonitor monitor)
    {
        ...
        try
        {
            ...
            NPC Create() => new(new AnimatedSprite("Characters/Jodi",0,16,32),Vector2.Zero,2,"Jodi"){currentLocation=location};
            ...
            foreach(var variant in new[]{"Base","Winter","Beach"})
            {
                ...
                var suffix=variant=="Base"?"":"_"+variant;var spriteAsset="Characters/Jodi"+suffix;var portraitAsset="Portraits/Jodi"+suffix;
                ...
                var texture=npc.Sprite.Texture;var portrait=npc.Portrait;var frameCount=variant=="Beach"?16:variant=="Base"?26:25;
                ...
                if(variant!="Beach")for(var i=variant=="Base"?26:25;i<28;i++){for(var y=0;y<32;y++)for(var x=0;x<16;x++)if(p[(i/4*32+y)*64+i%4*16+x]!=new Color(123,66,26,255))throw new Exception("Native solid placeholder differs");placeholderCount++;}
                for(var i=0;i<(variant=="Beach"?4:5);i++){if(new Dialogue(npc,null,"Expression.$"+i).getPortraitIndex()!=i)throw new Exception("Portrait index differs");var count=0;for(var y=0;y<64;y++)for(var x=0;x<64;x++)if(pp[(i/2*64+y)*128+i%2*64+x].A>0)count++;if(count==0)throw new Exception("Empty portrait");portraitCount++;}
                if(variant!="Beach")for(var y=128;y<192;y++)for(var x=64;x<128;x++)if(pp[y*128+x]!=new Color(94,83,255,255))throw new Exception("Native portrait placeholder5 changed");
                ...
                if(variant!="Beach")foreach(var key in new[]{"jodi_dishes","jodi_exercise","jodi_sit_down","jodi_sleep"})
                {
                ...
                try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<texture.Height/32*4;i++)batch.Draw(texture,new Rectangle(i%4*64+8,i/4*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(portrait,new Rectangle(300,16,384,portrait.Height*3),Color.White);batch.End();device.SetRenderTargets(previous);using var file=File.Create(Path.Combine(helper.DirectoryPath,"jodi-"+variant.ToLowerInvariant()+"-runtime-preview.png"));target.SaveAsPng(file,target.Width,target.Height);}finally{device.SetRenderTargets(previous);}
            }
            Game1.season=Season.Spring;npc.wearNormalClothes();npc.ChooseAppearance();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Jodi"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Jodi"));
            helper.Data.WriteJsonFile("jodi-checks.json",new{Passed=true,OccupiedPoses=poseCount,PreservedPlaceholders=placeholderCount,PortraitSlots=portraitCount,PreservedPortraitPlaceholders=2,NativeWalkingSteps=walkingSteps,NativeSpecialSteps=specialSteps,NativeRouteTransitions=routes,NativeWinterBeachTransitions=true,EverydayRestored=true,FarmLoaded=false,FullEventPlayback=false});monitor.Log("Jodi art audit passed; full saved-game scenes remain unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("jodi-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Jodi audit failed: "+ex,LogLevel.Error);}
