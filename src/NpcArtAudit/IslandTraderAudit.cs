...
internal sealed class IslandTraderAudit
{
    ...
    public IslandTraderAudit(IModHelper helper,IMonitor monitor){this.helper=helper;this.monitor=monitor;}
    public void Tick()
    {
        ...
        try
        {
            if(stage==0)
            {
                ...
                var forest=new IslandNorth();Game1.currentLocation=forest;
                forest.map=helper.GameContent.Load<xTile.Map>("Maps/Island_N");
                Utility.TryOpenShopMenu("IslandTrade", forest, playOpenSound: false);
                var shop=Game1.activeClickableMenu as ShopMenu??throw new Exception("Native Island Trader shop absent");
                if(shop.ShopId!="IslandTrade")throw new Exception("Wrong native shop");stock=shop.forSale.Count;greeting=shop.potraitPersonDialogue;stage=1;return;
            }
            var active=Game1.activeClickableMenu as ShopMenu??throw new Exception("Shop closed unexpectedly");
            if(active.ShopId!="IslandTrade"||active.portraitTexture==null||active.forSale.Count!=stock||active.potraitPersonDialogue!=greeting||stock==0)throw new Exception("Shop portrait, inventory or greeting mismatch");
            var expected=helper.GameContent.Load<Texture2D>("Portraits/IslandTrader");var a=new Color[active.portraitTexture.Width*active.portraitTexture.Height];var b=new Color[expected.Width*expected.Height];active.portraitTexture.GetData(a);expected.GetData(b);if(!a.SequenceEqual(b))throw new Exception("Wrong portrait pixels");
            ...
            try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);active.draw(batch);batch.End();device.SetRenderTargets(previousTargets);using var stream=File.Create(Path.Combine(helper.DirectoryPath,"island-trader-shop-preview.png"));target.SaveAsPng(stream,target.Width,target.Height);}
            finally{device.SetRenderTargets(previousTargets);}
            helper.Data.WriteJsonFile("island-trader-checks.json",new{Passed=true,NativeIslandTraderShop=true,PortraitPixelsMatch=true,StockPreserved=true,GreetingPreserved=true,AvailableItems=stock,WideScreenRendered=true,TransactionsTested=false,FarmLoaded=false});
            monitor.Log("Island Trader checks passed: native IslandTrade shop, matching portrait, inventory and greeting preserved; wide preview rendered.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("island-trader-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log($"Island Trader audit failed: {ex}",LogLevel.Error);}
