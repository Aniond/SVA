...
internal static class CarolineAudit
{
    public static void Run(IModHelper helper, IMonitor monitor)
    {
        ...
        try
        {
            ...
            NPC Create() => new(new AnimatedSprite("Characters/Caroline",0,16,32),Vector2.Zero,2,"Caroline"){currentLocation=location};
            var npc=Create();var walkingSteps=0;var specialSteps=0;var routes=0;var poseCount=0;var portraitCount=0;var placeholderCount=0;var teaShowFrameCommands=0;
            foreach(var variant in new[]{"Base","Winter","Beach"})
            {
                ...
                var suffix=variant=="Base"?"":"_"+variant;var spriteAsset="Characters/Caroline"+suffix;var portraitAsset="Portraits/Caroline"+suffix;
                ...
                if(texture.Width!=64||texture.Height!=(variant=="Beach"?128:224)||portrait.Width!=128||portrait.Height!=128)throw new Exception("Native atlas size differs");
                ...
                if(variant!="Beach")for(var i=25;i<28;i++){for(var y=0;y<32;y++)for(var x=0;x<16;x++)if(p[(i/4*32+y)*64+i%4*16+x]!=new Color(6,64,68,255))throw new Exception("Native solid placeholder differs");placeholderCount++;}
                ...
                if(variant!="Beach")foreach(var key in new[]{"caroline_exercise","caroline_read","caroline_sleep"})
                {
                ...
                if(variant!="Beach")
                {
                    ...
                    var tea=helper.GameContent.Load<Dictionary<string,string>>("Data/Events/Sunroom").First(p=>p.Key.StartsWith("719926/")).Value;
                    var teaFrames=new List<int>();
                    foreach(var command in tea.Split('/'))
                    {
                        var args=command.Split(' ',StringSplitOptions.RemoveEmptyEntries);if(args.Length<3||args[0]!="showFrame"||args[1]!="Caroline")continue;
                        var frame=int.Parse(args[2]);if(frame<0||frame>=25)throw new Exception("Tea event referenced invalid pose");npc.Sprite.StopAnimation();npc.Sprite.CurrentFrame=frame==0?1:0;
                        Event.DefaultCommands.ShowFrame(scene,args,new EventContext(scene,location,Game1.currentGameTime,args));if(npc.Sprite.CurrentFrame!=frame||npc.Sprite.SourceRect!=new Rectangle(frame%4*16,frame/4*32,16,32))throw new Exception("Tea ShowFrame differs");teaFrames.Add(frame);teaShowFrameCommands++;
                    }
                    if(!teaFrames.SequenceEqual(new[]{22,4,20,21,20,21,20,21,20,0}))throw new Exception("Native tea sequence changed");
                }
                ...
                try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<texture.Height/32*4;i++)batch.Draw(texture,new Rectangle(i%4*64+8,i/4*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(portrait,new Rectangle(300,16,384,384),Color.White);batch.End();device.SetRenderTargets(previous);using var file=File.Create(Path.Combine(helper.DirectoryPath,"caroline-"+variant.ToLowerInvariant()+"-runtime-preview.png"));target.SaveAsPng(file,target.Width,target.Height);}finally{device.SetRenderTargets(previous);}
            }
            npc.wearNormalClothes();npc.ChooseAppearance();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Caroline"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Caroline"));
            helper.Data.WriteJsonFile("caroline-checks.json",new{Passed=true,OccupiedPoses=poseCount,PreservedPlaceholders=placeholderCount,PortraitSlots=portraitCount,NativeWalkingSteps=walkingSteps,NativeSpecialSteps=specialSteps,NativeRouteTransitions=routes,NativeTeaShowFrameCommands=teaShowFrameCommands,NativeTeaSequenceMatched=true,NativeWinterBeachTransitions=true,EverydayRestored=true,FarmLoaded=false,FullEventPlayback=false});monitor.Log("Caroline art audit passed; full saved-game scenes remain unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("caroline-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Caroline audit failed: "+ex,LogLevel.Error);}
