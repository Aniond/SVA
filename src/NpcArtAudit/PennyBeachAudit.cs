...
internal static class PennyBeachAudit
{
    public static void Run(IModHelper helper,IMonitor monitor)
    {
        ...
        try
        {
            ...
            if((npc.Sprite.overrideTextureName??npc.Sprite.textureName.Value).Replace('\\','/')!="Characters/Penny_Beach")throw new Exception("Native beach appearance not selected");
            ...
            Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Penny_Beach"));
            Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Penny_Beach"));
            ...
            foreach(var section in animationData[key].Split('/').Take(3))
            {
                ...
                if(frames.Any(frame=>frame<0||frame>=20))throw new Exception("Native towel animation references empty or invalid frame");
            ...
            foreach(var key in new[]{"penny_beach_towel"})
            {
                var towelNpc=new NPC(new AnimatedSprite("Characters/Penny",0,16,32),Vector2.Zero,2,"Penny"){currentLocation=location};
                ...
                // Penny has laying_down metadata only; Maru's separate offset 0 16 does not apply.
                if(animationData[key].Split('/').Skip(4).Any(section=>section.StartsWith("offset ")))throw new Exception("Penny native towel metadata gained an offset");
            ...
            var walkingSteps = 0;
            foreach (var asset in new[] { "Characters/Penny_Beach" })
            {
            ...
            try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<20;i++)batch.Draw(texture,new Rectangle(i%8*64+8,i/8*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(npc.Portrait,new Rectangle(600,16,384,1344),Color.White);batch.End();device.SetRenderTargets(previous);using var f=File.Create(Path.Combine(helper.DirectoryPath,"penny-beach-runtime-preview.png"));target.SaveAsPng(f,target.Width,target.Height);}finally{device.SetRenderTargets(previous);}
            npc.wearNormalClothes();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Penny"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Penny"));
            helper.Data.WriteJsonFile("penny-beach-checks.json",new{Passed=true,OccupiedFrames=20,BlankFrames=0,PortraitSlots=14,NativeBeachSelected=true,NormalOutfitRestored=true,NativeTowelSteps=animationSteps,NativeWalkingSteps=walkingSteps,NativeTowelBehaviors=nativeTowelBehaviors,RecliningSceneVerified=false,FarmLoaded=false,FullEventPlayback=false});
            monitor.Log("Penny beach audit passed:20 poses,14 portrait slots, native beach selection and normal outfit restoration; event playback remains unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("penny-beach-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Penny beach audit failed: "+ex,LogLevel.Error);}
