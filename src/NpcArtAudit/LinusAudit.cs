...
internal static class LinusAudit
{
    public static void Run(IModHelper helper, IMonitor monitor)
    {
        ...
        try
        {
            ...
            NPC Create() => new(new AnimatedSprite("Characters/Linus",0,16,32),Vector2.Zero,2,"Linus"){currentLocation=location};
            var npc=Create();var walkingSteps=0;var specialSteps=0;var routes=0;var poseCount=0;var portraitCount=0;var placeholderCount=0;var eventShowFrameCommands=0;var rummageSteps=0;var eventAnimateCommands=0;
            foreach(var variant in new[]{"Base","Winter"})
            {
                ...
                var suffix=variant=="Base"?"":"_"+variant;var spriteAsset="Characters/Linus"+suffix;var portraitAsset="Portraits/Linus"+suffix;
                ...
                var texture=npc.Sprite.Texture;var portrait=npc.Portrait;var frameCount=27;
                ...
                if(variant!="Beach")for(var i=27;i<28;i++){for(var y=0;y<32;y++)for(var x=0;x<16;x++)if(p[(i/4*32+y)*64+i%4*16+x]!=Color.Transparent)throw new Exception("Native transparent placeholder differs");placeholderCount++;}
                ...
                if(variant!="Beach")foreach(var key in new[]{"linus_sleep"})
                {
                ...
                if(variant!="Beach")
                {
                    ...
                    foreach(var map in new[]{"BusStop","IslandSouth","Mountain","Town"})
                    ...
                    {
                        var args=command.Split(' ',StringSplitOptions.RemoveEmptyEntries);if(args.Length<3||args[0]!="showFrame"||args[1]!="Linus")continue;var frame=int.Parse(args[2]);if(frame<0||frame>=27)throw new Exception("Native event references missing pose");npc.Sprite.StopAnimation();npc.Sprite.CurrentFrame=frame==0?1:0;
                    ...
                    foreach(var expected in new[]{19,22,24,25,26})if(!seen.Contains(expected))throw new Exception("Native event pose missing: "+expected);
                    var punch=string.Join("/",new[]{"Mountain","Town"}.SelectMany(map=>helper.GameContent.Load<Dictionary<string,string>>("Data/Events/"+map).Values));
                    foreach(var command in punch.Split('/'))
                    {
                        var args=command.Split(' ',StringSplitOptions.RemoveEmptyEntries);if(args.Length<6||args[0]!="animate"||args[1]!="Linus")continue;
                        ...
                        if(!npc.Sprite.currentAnimation.Select(f=>f.frame).SequenceEqual(frames)||!npc.Sprite.currentAnimation.All(f=>f.milliseconds==duration))throw new Exception("Native rummage command differs");
                        npc.Sprite.CurrentFrame=frames[0];npc.Sprite.currentAnimationIndex=0;npc.Sprite.timer=0;
                        for(var step=1;step<=frames.Length*2;step++){npc.Sprite.animateOnce(new GameTime(TimeSpan.FromMilliseconds(step*duration),TimeSpan.FromMilliseconds(duration)));if(npc.Sprite.CurrentFrame!=frames[step%frames.Length])throw new Exception("Rummage timed transition differs");rummageSteps++;}
                ...
                try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<texture.Height/32*4;i++)batch.Draw(texture,new Rectangle(i%4*64+8,i/4*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(portrait,new Rectangle(300,16,384,portrait.Height*3),Color.White);batch.End();device.SetRenderTargets(previous);using var file=File.Create(Path.Combine(helper.DirectoryPath,"linus-"+variant.ToLowerInvariant()+"-runtime-preview.png"));target.SaveAsPng(file,target.Width,target.Height);}finally{device.SetRenderTargets(previous);}
            }
            Game1.season=Season.Spring;npc.wearNormalClothes();npc.ChooseAppearance();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Linus"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Linus"));
            helper.Data.WriteJsonFile("linus-checks.json",new{Passed=true,OccupiedPoses=poseCount,PreservedPlaceholders=placeholderCount,PortraitSlots=portraitCount,NativeWalkingSteps=walkingSteps,NativeSpecialSteps=specialSteps,NativeRouteTransitions=routes,NativeEventShowFrameCommands=eventShowFrameCommands,NativeRummageTimedSteps=rummageSteps,NativeEventAnimateCommands=eventAnimateCommands,NativeWinterTransitions=true,EverydayRestored=true,FarmLoaded=false,FullEventPlayback=false});monitor.Log("Linus art audit passed; full saved-game scenes remain unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("linus-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Linus audit failed: "+ex,LogLevel.Error);}
