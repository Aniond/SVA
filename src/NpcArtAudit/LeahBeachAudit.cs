...
internal static class LeahBeachAudit
{
    public static void Run(IModHelper helper,IMonitor monitor)
    {
        ...
        try
        {
            ...
            if((npc.Sprite.overrideTextureName??npc.Sprite.textureName.Value).Replace('\\','/')!="Characters/Leah_Beach")throw new Exception("Native beach appearance not selected");
            ...
            Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Leah_Beach"));
            Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Leah_Beach"));
            ...
            foreach(var section in helper.GameContent.Load<Dictionary<string,string>>("Data/animationDescriptions")["leah_beach_drink"].Split('/').Take(3))
            {
                var frames=section.Split(' ',StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();
                if(frames.Any(frame=>frame<0||frame>=20))throw new Exception("Native drink frame exceeds sheet");
                ...
                for(var step=1;step<=frames.Length*2;step++)
                {
                    ...
                    if(npc.Sprite.CurrentFrame!=frames[step%frames.Length])throw new Exception("Native drink playback mismatch");
                    animationSteps++;
                }
            }
            ...
            try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<20;i++)batch.Draw(texture,new Rectangle(i%8*64+8,i/8*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(npc.Portrait,new Rectangle(600,16,384,768),Color.White);batch.End();device.SetRenderTargets(previous);using var f=File.Create(Path.Combine(helper.DirectoryPath,"leah-beach-runtime-preview.png"));target.SaveAsPng(f,1024,1024);}finally{device.SetRenderTargets(previous);}
            npc.wearNormalClothes();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Leah"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Leah"));
            helper.Data.WriteJsonFile("leah-beach-checks.json",new{Passed=true,OccupiedFrames=20,BlankFrames=0,PortraitSlots=8,NativeBeachSelected=true,NormalOutfitRestored=true,NativeDrinkSteps=animationSteps,FarmLoaded=false,FullEventPlayback=false});
            monitor.Log("Leah beach audit passed:20 poses,8 portrait slots, native beach selection and normal outfit restoration; event playback remains unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("leah-beach-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Leah beach audit failed: "+ex,LogLevel.Error);}
