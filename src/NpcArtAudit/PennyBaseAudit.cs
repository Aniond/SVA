...
internal static class PennyBaseAudit
{
    public static void Run(IModHelper helper,IMonitor monitor)
    {
        ...
        try
        {
            ...
            if((npc.Sprite.overrideTextureName??npc.Sprite.textureName.Value).Replace('\\','/')!="Characters/Penny")throw new Exception("Native everyday appearance not selected");
            ...
            Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Penny"));
            Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Penny"));
            ...
            var walkingSteps = 0;
            foreach (var asset in new[] { "Characters/Penny", "Characters/Penny_Winter", "Characters/Penny_Beach" })
            {
            ...
            try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<52;i++)batch.Draw(texture,new Rectangle(i%8*64+8,i/8*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(npc.Portrait,new Rectangle(600,16,384,1344),Color.White);batch.End();device.SetRenderTargets(previous);using var f=File.Create(Path.Combine(helper.DirectoryPath,"penny-base-runtime-preview.png"));target.SaveAsPng(f,target.Width,target.Height);}finally{device.SetRenderTargets(previous);}
            Game1.season=Season.Winter;npc.ChooseAppearance();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Penny_Winter"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Penny_Winter"));
            helper.Data.WriteJsonFile("penny-base-checks.json",new{Passed=true,OccupiedArtworkFrames=49,BrownPlaceholderFrames=3,PortraitSlots=14,NativeEverydaySelected=true,WinterTransition=true,NativeDishesReadSitWaveSleepSteps=animationSteps,NativeWalkingSteps=walkingSteps,FarmLoaded=false,FullEventPlayback=false});
            monitor.Log("Penny everyday audit passed:49 poses,3 unchanged brown placeholders,14 portrait slots, native everyday selection and winter transition; event playback remains unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("penny-base-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Penny everyday audit failed: "+ex,LogLevel.Error);}
