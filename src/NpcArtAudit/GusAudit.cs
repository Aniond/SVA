...
// Verifies native walking, violin, cooking and cleaning, portraits and season changes.
internal static class GusAudit
{
    public static void Run(IModHelper helper, IMonitor monitor)
    {
        ...
        try
        {
            ...
            NPC Create() => new(new AnimatedSprite("Characters/Gus",0,16,32),Vector2.Zero,2,"Gus"){currentLocation=location};
            var npc=Create();var walkingSteps=0;var specialSteps=0;var routes=0;var poseCount=0;var portraitCount=0;var placeholderCount=0;var eventAnimationCommands=0;var eventAnimationSteps=0;
            foreach(var variant in new[]{"Base","Winter"})
            {
                ...
                var suffix=variant=="Base"?"":"_"+variant;var spriteAsset="Characters/Gus"+suffix;var portraitAsset="Portraits/Gus"+suffix;
                ...
                var texture=npc.Sprite.Texture;var portrait=npc.Portrait;var frameCount=28;
                ...
                if(variant!="Beach")foreach(var key in new[]{"gus_clean","gus_sit_down","gus_sleep"})
                {
                ...
                var eventSets=new List<int[]>();var scene=new Event();scene.actors.Add(npc);helper.Reflection.GetField<bool>(scene,"eventFinished").SetValue(true);
                foreach(var map in new[]{"Saloon","Farm"})foreach(var script in helper.GameContent.Load<Dictionary<string,string>>("Data/Events/"+map).Values)foreach(var command in script.Split('/'))
                {
                    var args=command.Split(' ',StringSplitOptions.RemoveEmptyEntries);if(args.Length<6||args[0]!="animate"||args[1]!="Gus")continue;var interval=int.Parse(args[4]);var frames=args.Skip(5).Select(int.Parse).ToArray();if(frames.Any(f=>f<0||f>=28||f==23))throw new Exception("Native event references missing Gus pose");eventSets.Add(frames);
                    Event.DefaultCommands.Animate(scene,args,new EventContext(scene,location,Game1.currentGameTime,args));if(!npc.Sprite.currentAnimation.Select(f=>f.frame).SequenceEqual(frames)||npc.Sprite.loop!=bool.Parse(args[3]))throw new Exception("Native Animate setup differs");for(var step=1;step<=frames.Length*2;step++){npc.Sprite.animateOnce(new GameTime(TimeSpan.FromMilliseconds(step*interval),TimeSpan.FromMilliseconds(interval)));if(npc.Sprite.CurrentFrame!=frames[step%frames.Length])throw new Exception("Event animation timed transition differs");eventAnimationSteps++;}eventAnimationCommands++;
                }
                if(!eventSets.Any(f=>f.SequenceEqual(new[]{16,17}))||!eventSets.Any(f=>f.SequenceEqual(new[]{24,25}))||!eventSets.Any(f=>f.SequenceEqual(new[]{26}))||!eventSets.Any(f=>f.SequenceEqual(new[]{22})))throw new Exception("Violin, cooking or cheer event missing");
                ...
                try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<texture.Height/32*4;i++)batch.Draw(texture,new Rectangle(i%4*64+8,i/4*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(portrait,new Rectangle(300,16,384,384),Color.White);batch.End();device.SetRenderTargets(previous);using var file=File.Create(Path.Combine(helper.DirectoryPath,"gus-"+variant.ToLowerInvariant()+"-runtime-preview.png"));target.SaveAsPng(file,target.Width,target.Height);}finally{device.SetRenderTargets(previous);}
            }
            Game1.season=Season.Spring;npc.wearNormalClothes();npc.ChooseAppearance();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/Gus"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/Gus"));
            helper.Data.WriteJsonFile("gus-checks.json",new{Passed=true,OccupiedPoses=poseCount,PreservedPlaceholders=placeholderCount,PortraitSlots=portraitCount,NativeWalkingSteps=walkingSteps,NativeSpecialSteps=specialSteps,NativeRouteTransitions=routes,NativeWinterTransition=true,NativeEventAnimationCommands=eventAnimationCommands,NativeEventAnimationSteps=eventAnimationSteps,NativeViolinAndCookingSequences=true,EverydayRestored=true,FarmLoaded=false,FullEventPlayback=false});monitor.Log("Gus art audit passed; full saved-game scenes remain unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("gus-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Gus audit failed: "+ex,LogLevel.Error);}
