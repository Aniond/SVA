...
internal static class LeoAudit
{
    public static void Run(IModHelper helper, IMonitor monitor)
    {
        ...
        try
        {
            ...
            NPC Create() => new(new AnimatedSprite("Characters/ParrotBoy",0,16,32),Vector2.Zero,2,"Leo"){currentLocation=location};
            ...
            foreach(var variant in new[]{"Base","Winter"})
            {
                ...
                var suffix=variant=="Base"?"":"_"+variant;var spriteAsset="Characters/ParrotBoy"+suffix;var portraitAsset="Portraits/ParrotBoy"+suffix;
                ...
                for(var i=25;i<28;i++){for(var y=0;y<32;y++)for(var x=0;x<16;x++)if(p[(i/4*32+y)*64+i%4*16+x]!=Color.White)throw new Exception("Native white placeholder changed");placeholderCount++;}
                ...
                foreach(var key in new[]{"leo_sleep","leo_sit_up"})
                {
                ...
                var showCount=0;var animateCount=0;
                foreach(var map in new[]{"IslandNorth","IslandSouth","IslandWest","Mountain"})
                foreach(var script in helper.GameContent.Load<Dictionary<string,string>>("Data/Events/"+map).Values)
                foreach(var command in Event.ParseCommands(script))
                {
                    var args=ArgUtility.SplitBySpaceQuoteAware(command);
                    if(args.Length<3||args[1]!="Leo")continue;
                    var context=new EventContext(scene,location,Game1.currentGameTime,args);
                    if(args[0]=="showFrame")
                    {
                        npc.Sprite.StopAnimation();Event.DefaultCommands.ShowFrame(scene,args,context);
                        var frame=int.Parse(args[2]);if(npc.Sprite.CurrentFrame!=frame||npc.Sprite.SourceRect!=new Rectangle(frame%4*16,frame/4*32,16,32))throw new Exception("Native Leo event pose differs");showCount++;
                    }
                    if(args[0]=="animate")
                    {
                        npc.Sprite.StopAnimation();Event.DefaultCommands.Animate(scene,args,context);
                        var frames=args.Skip(5).Select(int.Parse).ToArray();var duration=int.Parse(args[4]);
                        if(!npc.Sprite.currentAnimation.Select(f=>f.frame).SequenceEqual(frames)||!npc.Sprite.currentAnimation.All(f=>f.milliseconds==duration))throw new Exception("Native Leo event animation differs");
                        npc.Sprite.animateOnce(new GameTime(TimeSpan.FromMilliseconds(duration+1),TimeSpan.FromMilliseconds(duration+1)));animateCount++;
                    }
                }
                if(showCount!=12||animateCount!=3)throw new Exception("Expected native Leo event commands missing");
                helper.Data.WriteJsonFile("leo-"+variant.ToLowerInvariant()+"-event-checks.json",new{Passed=true,ActualShowFrameCommands=showCount,ActualAnimateCommands=animateCount,FullEventPlayback=false});
                ...
                try{device.SetRenderTarget(target);device.Clear(new Color(35,45,58));batch.Begin(samplerState:SamplerState.PointClamp);for(var i=0;i<texture.Height/32*4;i++)batch.Draw(texture,new Rectangle(i%4*64+8,i/4*132+8,48,96),new Rectangle(i%4*16,i/4*32,16,32),Color.White);batch.Draw(portrait,new Rectangle(300,16,384,portrait.Height*3),Color.White);batch.End();device.SetRenderTargets(previous);using var file=File.Create(Path.Combine(helper.DirectoryPath,"leo-"+variant.ToLowerInvariant()+"-runtime-preview.png"));target.SaveAsPng(file,target.Width,target.Height);}finally{device.SetRenderTargets(previous);}
            }
            Game1.season=Season.Spring;npc.wearNormalClothes();npc.ChooseAppearance();Equal(npc.Sprite.Texture,helper.GameContent.Load<Texture2D>("Characters/ParrotBoy"));Equal(npc.Portrait,helper.GameContent.Load<Texture2D>("Portraits/ParrotBoy"));
            helper.Data.WriteJsonFile("leo-checks.json",new{Passed=true,OccupiedPoses=poseCount,PreservedPlaceholders=placeholderCount,PortraitSlots=portraitCount,NativeWalkingSteps=walkingSteps,NativeSpecialSteps=specialSteps,NativeRouteTransitions=routes,NativeWinterTransitions=true,EverydayRestored=true,FarmLoaded=false,FullEventPlayback=false});monitor.Log("Leo art audit passed; full saved-game scenes remain unverified.",LogLevel.Info);
        }
        catch(Exception ex){helper.Data.WriteJsonFile("leo-checks.json",new{Passed=false,Error=ex.ToString()});monitor.Log("Leo audit failed: "+ex,LogLevel.Error);}
