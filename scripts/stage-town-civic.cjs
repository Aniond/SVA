...
(async()=>{
 ...
 const patches=JSON.parse(await fs.readFile(root+'/artifacts/town-civic-modern/install-manifest.json','utf8'));
 ...
 for(const patch of patches){
  ...
  if(!item){
   ...
   item={data:Buffer.from(baseline),baseline:Buffer.from(baseline),relative:existing?.File??'assets/TownCivic/'+patch.asset.replaceAll('/','-')+'.png',width:p.info.width,height:p.info.height,areas:existing?(existing.PatchAreas??(existing.PatchArea?[existing.PatchArea]:[{X:0,Y:0,Width:p.info.width,Height:p.info.height}])):[]};
 ...
 const manifest=JSON.parse(await fs.readFile(src+'/manifest.json','utf8'));manifest.Version='0.8.6';manifest.Description='Polished NPCs, terrain, props, weather, town homes and seasonal civic buildings including restored, warehouse and theater appearances.';await fs.writeFile(src+'/manifest.json',JSON.stringify(manifest,null,2));
 const project=await fs.readFile(src+'/AbigailModern.csproj','utf8');await fs.writeFile(src+'/AbigailModern.csproj',project.replace(/<Version>[^<]+<\/Version>/,'<Version>0.8.6</Version>'));
 await fs.writeFile(root+'/artifacts/town-civic-modern/staging.json',JSON.stringify({version:'0.8.6',registeredTextures:registry.length,proof,installed:false},null,2));
 console.log(JSON.stringify({version:'0.8.6',registeredTextures:registry.length,updatedAssets:outputs.size}));
})();
