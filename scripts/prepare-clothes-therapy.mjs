...
const actor='ClothesTherapyCharacters';
const variant='Base';const partial=process.argv.includes('--partial');
const folder=root+'work/'+actor+'/';
...
async function grid(file,cols,rows){
 ...
 let transparentPixels=0;for(let i=3;i<s.data.length;i+=4)if(s.data[i]===0)transparentPixels++;const useAlpha=transparentPixels>s.info.width*s.info.height*.1;
 for(let p=0;p<s.data.length;p+=4){const [r,g,b,a]=s.data.subarray(p,p+4);if(a<160||(!useAlpha&&(r>180&&b>180&&g<70)))s.data.fill(0,p,p+4);else s.data[p+3]=255;}
...
const report={actor,variant,preparedOnly:true,partial,installed:false,frames:[],portraits:[],gridBounds:{}};
for(const [kind,cellW,cellH,outputName] of [['Characters',16,32,'characters'],['Portraits',64,64,'portraits']]){
 ...
 for(let frame=0;frame<cols*native.info.height/cellH;frame++){
  ...
  }else if(kind==='Characters'&&maxX>=0&&!placeholder&&!partial)throw new Error('Unmapped real sprite '+frame);
 ...
 const png=await sharp(output,{raw:native.info}).png().toBuffer();await fs.writeFile(folder+outputName+(partial?'-partial.png':'-prepared.png'),png);await sharp(png).resize({width:native.info.width*(kind==='Characters'?8:4),kernel:'nearest'}).png().toFile(folder+outputName+(partial?'-partial-preview.png':'-preview.png'));report[outputName+'Sheet']={width:native.info.width,height:native.info.height,sha256:sha(png)};
}
for(const [file,s]of Object.entries(grids))report.gridBounds[file]={x:s.xs,y:s.ys};
await fs.writeFile(folder+(partial?'partial-qa.json':'qa.json'),JSON.stringify(report,null,2));console.log(JSON.stringify({actor,variant,characters:report.charactersSheet,portraits:report.portraits.filter(p=>p.changed).length,realSprites:report.frames.filter(p=>p.changed).length,preserved:report.frames.filter(p=>!p.changed).map(p=>p.frame)},null,2));
