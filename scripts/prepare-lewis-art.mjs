...
const variant=process.argv[2]??'Winter',dir=`artifacts/npc-modern/work/Lewis${variant}/`,suffix=variant==='Base'?'':'_'+variant;
const raw=async p=>sharp(p).ensureAlpha().raw().toBuffer({resolveWithObject:true});
const original=await raw(`artifacts/npc-modern/originals/Characters/Lewis${suffix}.png`),output=Buffer.from(original.data),mapping=[];
const cache=new Map();
async function source(file,cols,rows){
 const p=dir+file;if(cache.has(p))return cache.get(p);
 const s=await raw(p);for(let i=0;i<s.data.length;i+=4){const [r,g,b,a]=s.data.subarray(i,i+4);if(a<160||(variant==='Beach'?(r>g+90&&b>g+90&&r>b*.92):(r>g+90&&b>g+90&&r>b*.8)))s.data.fill(0,i,i+4);else s.data[i+3]=255;}
...
let plan=[];
if(variant==='Winter')plan=[['walk-source.png',4,4,0,16],['special-source.png',4,3,16,12],['special-correction-source.png',4,2,28,1]];
if(variant==='Base')plan=[['walk-source.png',4,4,0,16],['special-source.png',4,3,16,12],['special-correction-source.png',4,2,28,1]];
...
const limit=original.info.height/32*4,placeholders=[];for(let f=29;f<limit;f++){for(let y=0;y<32;y++){const p=((Math.floor(f/4)*32+y)*64+f%4*16)*4;assert.ok(output.subarray(p,p+64).equals(original.data.subarray(p,p+64)));}placeholders.push(f);}
...
if(process.argv.includes('--portraits')){
 const native=await raw(`artifacts/npc-modern/originals/Portraits/Lewis${variant==='Beach'?'':suffix}.png`),pout=Buffer.alloc(native.data.length),pm=[];

 const accepted=variant!=='Winter'?await raw('src/AbigailModern/assets/Lewis/portraits.png'):null;
 for(let index=0;index<native.info.height/64*2;index++){
  if(accepted&&index<6){for(let y=0;y<64;y++){const p=((Math.floor(index/2)*64+y)*128+index%2*64)*4;accepted.data.copy(pout,p,p,p+256);}pm.push({index,file:'src/AbigailModern/assets/Lewis/portraits.png',retained:true});continue;}
  const file=variant==='Base'?'portraits-fill-source.png':index<4?'portraits-0-3-source.png':'portraits-tail-source.png',sourceIndex=variant==='Base'?0:index<4?index:index-4,data=await cell(file,2,2,sourceIndex,64,64);
 ...
 qa.nativePortraitAsset=variant==='Winter'?'Portraits/Lewis_Winter':'Portraits/Lewis';qa.portraitFallbackOnly=variant==='Beach';qa.portraitWidth=native.info.width;qa.portraitHeight=native.info.height;qa.portraitCount=native.info.height/64*2;qa.portraitSha256=crypto.createHash('sha256').update(await fs.readFile(dir+'portraits-prepared.png')).digest('hex');await fs.writeFile(dir+'qa.json',JSON.stringify(qa,null,2));
}
