...
(async()=>{
 ...
 const patches=JSON.parse(await fs.readFile(root+'/artifacts/town-houses-modern/install-manifest.json','utf8'));
 ...
 const manifest=JSON.parse(await fs.readFile(src+'/manifest.json','utf8'));manifest.Version='0.8.5';manifest.Description='Polished NPCs, terrain, props, weather, town shops and seasonal town homes including rebuilt Pam house.';await fs.writeFile(src+'/manifest.json',JSON.stringify(manifest,null,2));
 const project=await fs.readFile(src+'/AbigailModern.csproj','utf8');await fs.writeFile(src+'/AbigailModern.csproj',project.replace(/<Version>[^<]+<\/Version>/,'<Version>0.8.5</Version>'));
 await fs.writeFile(root+'/artifacts/town-houses-modern/staging.json',JSON.stringify({version:'0.8.5',registeredTextures:registry.length,proof,installed:false},null,2));
 console.log(JSON.stringify({version:'0.8.5',registeredTextures:registry.length,updatedAssets:outputs.size}));
})();
