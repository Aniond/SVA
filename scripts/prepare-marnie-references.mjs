...
for(const variant of variants){const work=`artifacts/npc-modern/work/Marnie${variant}/`;await fs.mkdir(work,{recursive:true});const suffix=variant==='Base'?'':'_'+variant;const inv={};
 for(const category of ['Characters','Portraits']){const file=`artifacts/npc-modern/originals/${category}/Marnie${suffix}.png`,{data,info}=await sharp(file).ensureAlpha().raw().toBuffer({resolveWithObject:true});const w=category==='Characters'?16:64,h=category==='Characters'?32:64,cols=info.width/w,cells=[];
...
for(const category of ['characters','portraits'])await sharp(`src/AbigailModern/assets/Marnie/${category}.png`).resize({width:category==='characters'?384:768,kernel:'nearest'}).png().toFile(`artifacts/npc-modern/work/MarnieBase/modern-${category}-reference.png`);
