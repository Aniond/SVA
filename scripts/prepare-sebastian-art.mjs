...
const variant=process.argv[2]??'Winter',dir=`artifacts/npc-modern/work/Sebastian${variant}/`,suffix=variant==='Base'?'':'_'+variant;
const raw=async p=>sharp(p).ensureAlpha().raw().toBuffer({resolveWithObject:true});
const original=await raw(`artifacts/npc-modern/originals/Characters/Sebastian${suffix}.png`),output=Buffer.from(original.data),mapping=[];
const cache=new Map();
...
let plan=[];
if(variant==='Winter')plan=[['walk-source.png',4,4,0,16],['special-source.png',4,4,16,16],['final-upper-correction-source.png',4,3,32,12],['final-lower-source.png',4,4,44,13]];
if(variant==='Base')plan=[['walk-source.png',4,4,0,16],['special-source.png',4,4,16,16],['final-upper-source.png',4,3,32,12],['final-lower-source.png',4,4,44,12],['sleep-source.png',1,1,56,1]];
if(variant==='Beach')plan=[['walk-source.png',4,4,0,16],['special-source.png',4,2,16,8]];
for(const [file,cols,rows,start,count]of plan)for(let i=0;i<count;i++){
 ...
 if(variant==='Winter'&&f===56){sourceFile='sleep-source.png';sourceCols=sourceRows=1;sourceIndex=0;}
...
const limit=original.info.height/32*4,placeholders=[];for(let f=57;f<limit;f++){for(let y=0;y<32;y++){const p=((Math.floor(f/4)*32+y)*64+f%4*16)*4;assert.ok(output.subarray(p,p+64).equals(original.data.subarray(p,p+64)));}placeholders.push(f);}
...
if(process.argv.includes('--portraits')){
 const native=await raw(`artifacts/npc-modern/originals/Portraits/Sebastian${suffix}.png`),pout=Buffer.alloc(native.data.length),pm=[];
 if(variant==='Base'){
  const existing=await raw('src/AbigailModern/assets/Sebastian/portraits.png');assert.equal(existing.info.width,native.info.width);assert.equal(existing.info.height,native.info.height);existing.data.copy(pout);pm.push({all:10,source:'src/AbigailModern/assets/Sebastian/portraits.png',note:'Accepted complete main portrait art retained byte-for-byte.'});
 }else for(let index=0;index<native.info.height/64*2;index++){
