...
const variant=process.argv[2]??'Winter',dir=`artifacts/npc-modern/work/Pierre${variant}/`,suffix=variant==='Base'?'':'_'+variant;
const raw=async p=>sharp(p).ensureAlpha().raw().toBuffer({resolveWithObject:true});
const original=await raw(`artifacts/npc-modern/originals/Characters/Pierre${suffix}.png`),output=Buffer.from(original.data),mapping=[];
const cache=new Map();
...
let plan=[];
if(variant==='Winter')plan=[['walk-source.png',4,4,0,16],['special-source.png',3,3,16,9]];
if(variant==='Base')plan=[['walk-source.png',4,4,0,16],['special-source.png',3,3,16,9]];
...
for(const [file,cols,rows,start,count]of plan)for(let i=0;i<count;i++){
 ...
 if(f===3||f===11){sourceIndex=f-2;flip=true;}
...
const limit=original.info.height/32*4,placeholders=[];for(let f=25;f<limit;f++){for(let y=0;y<32;y++){const p=((Math.floor(f/4)*32+y)*64+f%4*16)*4;assert.ok(output.subarray(p,p+64).equals(original.data.subarray(p,p+64)));}placeholders.push(f);}
...
if(process.argv.includes('--portraits')){
 const native=await raw(`artifacts/npc-modern/originals/Portraits/Pierre${suffix}.png`),pout=Buffer.alloc(native.data.length),pm=[];

 const accepted=variant==='Base'?await raw('src/AbigailModern/assets/Pierre/portraits.png'):null;
 for(let index=0;index<native.info.height/64*2;index++){
  if(accepted&&index<5){for(let y=0;y<64;y++){const p=((Math.floor(index/2)*64+y)*128+index%2*64)*4;accepted.data.copy(pout,p,p,p+256);}pm.push({index,file:'src/AbigailModern/assets/Pierre/portraits.png',retained:true});continue;}
