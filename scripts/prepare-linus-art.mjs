...
const variant=process.argv[2]??'Winter',dir=`artifacts/npc-modern/work/Linus${variant}/`,suffix=variant==='Base'?'':'_'+variant;
const raw=async p=>sharp(p).ensureAlpha().raw().toBuffer({resolveWithObject:true});
const original=await raw(`artifacts/npc-modern/originals/Characters/Linus${suffix}.png`),output=Buffer.from(original.data),mapping=[];
const cache=new Map();
async function source(file,cols,rows){
 const p=dir+file;if(cache.has(p))return cache.get(p);
 const s=await raw(p);for(let i=0;i<s.data.length;i+=4){const [r,g,b,a]=s.data.subarray(i,i+4);if(a<160||(r>g+90&&b>g+90&&r>b*.8))s.data.fill(0,i,i+4);else s.data[i+3]=255;}
...
let plan=[];
if(variant==='Winter')plan=[['walk-source.png',4,4,0,16],['legacy-source.png',4,1,16,3],['special-source.png',4,2,19,8]];
if(variant==='Base')plan=[['walk-repair-source.png',2,3,0,16],['legacy-source.png',4,1,16,3],['special-source.png',4,2,19,8]];
...
for(const [file,cols,rows,start,count]of plan)for(let i=0;i<count;i++){
 ...
 if(maxX<0)continue;
 if(variant==='Base'&&[0,1,2,3].includes(f)){const accepted=await raw('src/AbigailModern/assets/Linus/characters.png');for(let y=0;y<32;y++){const p=((fy+y)*64+fx)*4;accepted.data.copy(output,p,p,p+64);}mapping.push({frame:f,file:'src/AbigailModern/assets/Linus/characters.png',retained:true});continue;}
...
const limit=original.info.height/32*4,placeholders=[];for(let f=27;f<limit;f++){for(let y=0;y<32;y++){const p=((Math.floor(f/4)*32+y)*64+f%4*16)*4;assert.ok(output.subarray(p,p+64).equals(original.data.subarray(p,p+64)));}placeholders.push(f);}
...
if(process.argv.includes('--portraits')){
 const native=await raw(`artifacts/npc-modern/originals/Portraits/Linus${suffix}.png`),pout=Buffer.alloc(native.data.length),pm=[];

 const accepted=variant==='Base'?await raw('src/AbigailModern/assets/Linus/portraits.png'):null;
 for(let index=0;index<native.info.height/64*2;index++){
  if(accepted&&[0,2,3].includes(index)){for(let y=0;y<64;y++){const p=((Math.floor(index/2)*64+y)*128+index%2*64)*4;accepted.data.copy(pout,p,p,p+256);}pm.push({index,file:'src/AbigailModern/assets/Linus/portraits.png',retained:true});continue;}
  const file=variant==='Base'&&index===1?'portraits-happy-close-source.png':variant==='Winter'&&index<4?'portraits-0-3-source.png':'portraits-repair-source.png',sourceIndex=index>=4?index-3:variant==='Base'?0:index,data=await cell(file,2,2,sourceIndex,64,64);
