...
const work='artifacts/npc-modern/work/EvelynWinter/';
const bounds={};
...
const native=await sharp('artifacts/npc-modern/originals/Characters/Evelyn_Winter.png').ensureAlpha().raw().toBuffer({resolveWithObject:true});
assert.equal(native.info.width,64);assert.equal(native.info.height,192);
const output=Buffer.from(native.data),walk=await sheet('walk-source.png',4,4),special=await sheet('special-source.png',3,2),stride=await sheet('stride-source.png',2,1);
for(let frame=0;frame<21;frame++){
 ...
 let data=await cell(frame<16?walk:special,frame<16?frame:frame-16,width,height);
 if(frame===3)data=await sharp(await cell(walk,1,width,height),{raw:{width,height,channels:4}}).flop().raw().toBuffer();
...
if(!process.argv.includes('--sprites-only')){
 const source=await sheet('portraits-source.png',2,2),parts=[];
 for(let i=0;i<4;i++){const data=await cell(source,i,62,62);parts.push({input:await sharp(data,{raw:{width:62,height:62,channels:4}}).png().toBuffer(),left:i%2*64+1,top:Math.floor(i/2)*64+2});}
...
console.log(`Prepared 21 Evelyn winter poses, three exact native placeholders, ${portraits} portraits.`);
